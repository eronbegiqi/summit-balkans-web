...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.insert(bookings).values({
      ...
      paymentType: data.paymentOption === "full" ? "FULL" : "DEPOSIT",
      ...
      status: "NEW",
      internalNotes: `Payment: ${data.paymentOption} (€${data.paymentAmount}) | Add-ons: ${data.addOns.join(", ")} | Dep: ${data.departureId} | Guide: ${data.guide}`,
    ...
    await Promise.all([
      getResend().emails.send({
        from: FROM,
        to: data.email,
        subject: `Spot reserved — ${data.tourName} · ${bookingRef}`,
      ...
      getResend().emails.send({
        ...
        to: ADMIN,
        replyTo: data.email,
        subject: `New booking ${bookingRef} — ${fullName} · ${data.tourName} · ${data.paymentOption === "full" ? "full" : "deposit"}`,
